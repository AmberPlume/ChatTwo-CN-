...
public class MessageManager : IAsyncDisposable
{
    ...
    private void ProcessMessage(PendingMessage pendingMessage)
    {
        ...
        if (CensorFilter.IsAvailable && chatCode.IsPlayerMessage())
        {
            pendingMessage.Content = CensorFilter.ProcessContent(pendingMessage.Content);
            pendingMessage.Sender = CensorFilter.ProcessContent(pendingMessage.Sender);
        }
