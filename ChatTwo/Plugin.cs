...
public sealed class Plugin : IDalamudPlugin
{
    ...
    // → detour 无触发时机。主线程在 hover 状态沿主动调一次 SetCursor：上升沿设手指句柄、
    // 下降沿设箭头句柄。游戏静止不覆盖 → 设置后保持、不闪烁。
    ...
    /// !!! 游戏在 ImGui 窗口（聊天框）上方静止时不调 user32.SetCursor →
    ...
    //（用 GetProcAddress 拿导出地址，非游戏模块内偏移）
    ...
    // !!! CalcTextSize hook（布局放大）会触发 ImGui Fallback "Debug##Default" 窗口，绘制层无副作用。
    // !!! 结构（勿改前提，来自 DalamudIme.Draw）：
    ...
    // 候选框锚点 = 原始框左下角（pMin.X, pMax.Y - offset）——映射基准不含拼音空隙
    ...
    private static unsafe void AddRectFilledDetour(nint drawList, System.Numerics.Vector2 pMin, System.Numerics.Vector2 pMax, uint col, float rounding, uint flags)
    {
        ...
        if (_candidatePrev && _imeActive && drawList == _foregroundDl && _imeScale > 1.001f)
        {
            ...
            var offset = _imePinyinTopPrev > 0
                ...
                : 0f;
            _imeOffset = offset;
            var yCoeff = 0.5f * _imeScale;  // 行距系数 0.5
    ...
    public Plugin()
    {
        try
        {
            ...
            // 指针直接消失（ImGui 的 None 会调 SetCursor(NULL) 隐藏）。NoMouseCursorChange
            ...
            // 频道切换策略：灵活模式（体感灵活，保留）。
            // !!! 原版遗留：L133 曾有"热键固定严格模式"覆盖行，与下面的 Flexible 冲突（后写覆盖先写），
            // 且与"按住 W 移动时也能切频道"矛盾 → 已删除 Strict 覆盖行。
            ...
            // !!! 清理：PrettierTimestamps/MoreCompactPretty/HideSameTimestamps（原作者
            ...
            // !!! 构造函数直接 Load：懒加载时若未设 _tp，EnsureLoaded 永远不加载 → 回退 FontAwesome（图标消失）。
    ...
    private unsafe void Draw()
    {
        ...
        // 否则卫月标题栏等窗口 chrome 会随主字体缩放（还会挡设置页保存按钮）。
    ...
    private void FrameworkUpdate(IFramework framework)
    {
        ...
        // !!! 已移除"屏幕外可见"hack，直接验证 OwnerAddon=0（bindToOwner=false 等效）。
        ...
        // addon 根本不会显示。两种情况都会导致原生聊天框永不隐藏。
        // 菜单不可见（打开失败或已关闭）→ 恢复隐藏。
