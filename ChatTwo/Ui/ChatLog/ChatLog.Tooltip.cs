...
{
    // OpenAddonByAgent hook（vtable 22）：OpenContextMenu 内部会把 BlockedParentId 清成 0，
    // Dalamud OnMenuOpened 的 AddonName = GetAddonById(BlockedParentId) 读到空 → DR/Allagan
    // 的 switch 不识别 → 道具/玩家菜单项不注入。本 detour 在 Dalamud 之前执行（后 hook 链上前置）：
    // 先设 BlockedParentId=ChatLog，再调链 → Dalamud 读到 "ChatLog" → 插件项正常注入。
    // 仅 ChatTwo 菜单会话（ContextMenuActive）且打开 "ContextMenu" 时设置，不影响背包等原生场景。
    ...
    // SetPosition hook：拦截游戏对 ItemDetail/ActionDetail 的每帧位置覆盖。
    ...
    /// <summary>
    /// ContextMenu（一级菜单）的 PreDraw 回调，每帧执行。不再控制菜单位置（恢复游戏原生跟手，
    /// 由 PayloadHandler 打开时 SetPosition 定位）。保留逻辑：
    /// ① OwnerAddon 清零（时分复用：注入阶段=ChatLog 供 DR/Allagan 识别，渲染阶段清 0 防二级菜单绑定）；
    /// ② BlockedParentId 清零（注入阶段=ChatLog 供 AddonName 识别，渲染阶段清 0 防菜单闪没）；
    /// ③ 会话状态复位（菜单隐藏时清 ContextMenuActive / ChatTwoMenuSession，防残留穿透）。
    /// 仅 ContextMenuActive 为 true（菜单由 ChatTwo 触发）时执行。
    /// </summary>
